    ...
    breakList.map(requestItem => {
      ...
      item = {
        ...
        exampleParams: requestItem.request.body
          ? ParamsDataClear(requestItem.request.body.raw)
          : null,
        url: VariableToString(
          requestItem.request.url.raw,
          requestItem.request.url.host[0]
        ),
...
export const RESQUEST_API = list => {
  ...
  list.map(item => {
    ...
    ${
      item.exampleParams
        ? "* @param data " + JSON.stringify(item.exampleParams)
        : ""
    }*/
    ${item.name}: (${
      item.sendParams ? "data" : ""
